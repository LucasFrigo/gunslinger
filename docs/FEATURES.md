...
| Regional hit effects | `done` | Head instakill; torso/limb 1 HP (default HP 2); surviving arm hit flings the revolver (`gun_pain_toss_up` / `gun_pain_toss_spin`); leg slows move. MP host HP + `_mp_wound` |
| Pain-jerk disarm | `done` | Arm hit while the gun is held calls `WeaponBase.pain_jerk_into_world` (short upward toss). VR catches with either hand (`gun_catch_radius`). Flat: look at the loose gun and press RMB (`draw_toggle`) to grab it in the air or on the ground (`gun_flat_grab_range` / `gun_flat_grab_radius`); RMB does not hip-snap a loose gun. No redraw lock. AI uses the same toss and snatches it back after `arm_disarm_duration`. Pose stream already carries the free gun |
...
| Ragdoll physics | `planned` | Roadmap (Medium); lethal hit drops the posed corpse into `PhysicalBone3D` sim. Needs a skeleton on `gunslinger.glb` (A-pose, unwired today) |
| Dismemberment | `planned` | Roadmap (Medium); pre-cut boolean chunks in `gunslinger.blend` hide/swap head, arm, or leg off the existing hitboxes. Severed piece is a rigid body |
...
| Oil-field train | `planned` | Roadmap (Medium); variant of the moving train: desert crossing, pumpjacks beside the track, flaming oil bursts on the horizon |
