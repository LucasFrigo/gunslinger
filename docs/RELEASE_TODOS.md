...
- [ ] Off-hand props on the range: throw the cigarette into a bottle on the way out and on the way back (it shatters, cig comes home) and into a porch post (cig comes home early). Equip the ace, charge-throw it, pick it back up with F / grip; reselect the wedge and confirm the old card vanishes. Equip the coin: on flat it stays in hand until **G** flips it up onto a face or onto the floor (it must not clip through). VR: tilt the palm to drop it, then pick it up. Equip the bottle, charge-throw it into a rail or a porch post (it shatters, glass burst, no respawn, HUD count unchanged); set one down gently and pick it back up with F / grip; shoot one in the hand; reselect the wedge for a fresh bottle. Confirm a rail bottle still respawns on its spot. `from:` cigarette flight collision / coin toss / ace of spades / bottle misc prop
- [ ] Tutorial billboard: turn around on the porch; the news board on the back wall lists the current binds (flat keys vs VR controller words, no jam line in VR). Rebind cock in Settings and confirm the board updates. Readable at dawn and dusk. `from:` tutorial billboard
