...
- **Main menu SP / MP split:** Landing screen is Singleplayer and Multiplayer buttons (plus existing Settings / Quit). Singleplayer opens the current gauntlet + free-duel controls; Multiplayer opens the current LAN / Steam host-join UI. Same `ui/main_menu.tscn` / `ui/main_menu.gd` — a page switch like Settings already uses (`show_mode_select`), not new modes. LAN/Steam browse only while the MP page is open.
