...
- **Off-hand props (cigarette first):** Hold a random miscellaneous object in the off-hand while the gun stays in the gun-hand. First prop: a cigarette (mesh + simple hold/drop or mouth pose). Reuses VR grip / either-hand attach patterns from gun release; must not block off-hand reload (`ReloadProbe` / belt). Flat can stub or skip. More props later (bottle, badge, etc.) once the hold slot exists. MP avatar sync can follow.
