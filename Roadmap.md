...
- **Coin toss:** Off-hand prop (`props/coin.gd`, `assets/models/props/msc_coin.glb`). Flat G flips it up onto a face or onto the floor as a rigid body; VR still drops on a tilted palm. Local-only. (`done` in FEATURES)
- **Cigarette flight collision:** Thrown cig (`props/cigarette.gd` / `props/prop_flight.gd`) raycasts world + bottles on the outbound and return legs. A wall skips the hover and homes; a practice bottle shatters on either pass. Held mesh stays shapeless. (`done` in FEATURES)
- **Tutorial billboard:** News board on the practice-hub porch back wall (`practice/tutorial_board.tscn`) listing draw, fire, cock, reload, jam clear, props, bottles, and the slot lever. Same board in flat and VR; binds follow `PlayerSettings` remaps. (`done` in FEATURES)
