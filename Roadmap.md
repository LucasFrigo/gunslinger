...
* **VR cock on stick-down:** Default VR: pull the gun-hand analog down to cock the hammer. Today that stick axis starts the Ocelot spin (`player.gd` `_update_vr_spin`, `spin_stick_threshold`); cock is gun-hand A (`ax_button` in `player/vr_rig.gd`). Move the trick-shot / spin onto the old cock button (A). Update the README controls table when it ships. Flat Space cock stays.
* **Button remapping:** Player-facing bind table in Settings (`ui/settings_menu.tscn`) persisted in `PlayerSettings` / `user://settings.cfg`. Cover VR (cock, trick shot, gate, fire, grip) and flat (cock, fire, reload, draw). Defaults match the stick-down cock / A trick-shot layout above. No remapping exists today.
