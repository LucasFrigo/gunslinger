...
2. **More off-hand props:** Extra radial wedges beyond the cigarette — bottle, badge, etc. Each needs a mesh vendored next to `assets/models/props/msc_cigarette.glb` and an entry in `PropController.ITEMS`; the wheel, attach, and stow plumbing already exist. Props are local-only today: remote avatars show nothing in the off hand, so MP pose sync (a flag plus the equipped id) is the other half. Cigarette polish that is still open: an ember emission / tiny light on the tip, and a readability pass on the thrown cig (real 8.4 cm scale is nearly invisible in flight — see `cig_spin_axis`). Flight collision is its own item.
3. **Cigarette flight collision:** The thrown cigarette (`props/cigarette.gd`) has no collision today. The held mesh stays shapeless so it never blocks `ReloadProbe` / belt overlaps; only the flight tests hits. A wall or other solid returns it immediately (skip the far-end hover and head home). A practice bottle shatters (`practice/practice_bottle.gd` `shatter`, same break as a bullet or a hard throw).
4. **Coin toss:** Off-hand miscellaneous prop. A manual throw (VR off-hand trigger, flat `prop_fire` / `G` — same bind as the cigarette) launches the coin into a spin. Catch takes no button: the coin rests on the hand only if the player balances it there (palm under it, held steady and level). A bad angle or a moving hand lets it fall. The mesh needs both faces readable — heads on one side, tails on the other — vendored beside `assets/models/props/msc_cigarette.glb`, plus a wedge in `PropController.ITEMS`. Local-only, same as the other props.
5. **Ace of spades:** Off-hand miscellaneous prop. A poker card whose face reads as the ace of spades, vendored beside `assets/models/props/msc_cigarette.glb`, plus a wedge in `PropController.ITEMS`. Throw matches the cigarette (`prop_fire`: hold to charge, release to launch, same spin) but it does not boomerang home — it stays where it lands. Flight collision matches the cigarette item: a wall or solid stops it, a practice bottle shatters. Local-only, same as the other props.
6. **More enemy NPCs:** Expand the roster beyond Drunk / Sheriff / Ghost. New opponents are new `ai/archetypes/*.tres` (`AIArchetype` — reaction, accuracy, draw, reload, move style). Wire them into the free-duel pick and gauntlet ladder rungs. Distinct silhouettes/meshes can follow once the gunslinger model is on AI. First design: [Half-head](docs/design/characters/half-head.md).
7. **Tutorial billboard:** A news-board panel in the practice hub (`scenarios/practice_hub/`) that spells out button mappings and the mechanics (draw, fire, cock, reload, jam clear, off-hand props, bottle grab / throw, slot lever). Reads like an old-town newspaper billboard, not a settings screen. Same board in flat and VR; the listed binds follow remaps in `PlayerSettings` so it stays true after Settings. The porch back wall or the berm is the natural spot.
8. **Duel vs up to 3 NPCs:** Free-duel option to face 1–3 AI opponents in one standoff (local, no netcode). Needs extra spawn marks, multi-combatant targeting, and resolve when more than two duelists fire. Reuses `ai/duelist.tscn` + archetypes.
9. **Death cam (3rd-person corpse):** On a lethal hit, freeze the dead player (no move / fire / draw). Camera becomes a third-person orbit around their body; look/stick only orbits until the next duel starts. After `death_cam_hold` (~2s, tweakable) play the duel-replay item, then return to this orbit until rematch. SP + 1v1 MP. Replaces or sequences after the current trail fly-along KillCam (`autoload/kill_cam.gd`).
10. **Train Map Concept:** A duel scene featuring a moving train passing between opponents. Players must either wait for the train to clear or attempt risky shots through open train cars. Train Rooftop already scrolls desert and horizon past a stopped train; this item is the train that crosses the lane.
11. **Oil-field train:** A variant of the Train Map Concept. The same train crosses the lane, this time through open desert, with oil pumps (pumpjacks) scattered along both sides of the track. On the horizon, oil bursts burn as distant flame columns — a `VfxCatalog` cue, cosmetic only (local, no gameplay). Depends on the crossing train.
12. **Ragdoll physics:** On a lethal hit, the corpse collapses instead of holding the last pose the death cam orbits. `assets/models/characters/gunslinger.glb` is still an unskinned A-pose (not on AI or the remote avatar), so this needs a skeleton plus Godot `PhysicalBone3D` / `PhysicalBoneSimulator3D` against arena collision. At the lethal hit, drop the controller / IK pose and let the sim take the body; an impulse from the shot direction sells the fall. SP first; 1v1 MP can share a host-authored impulse so both peers see the same collapse. Depends on the death-cam corpse.
13. **Dismemberment:** A killing or limb-removing hit severs that part (head, either arm, a leg), keyed off the existing regions (`HeadHitbox`, `ArmHitboxL` / `ArmHitboxR`, `LegHitbox` in `player/player.gd`). Cost-effective path: pre-cut boolean meshes in `gunslinger.blend` that hide or swap out a chunk, rather than runtime CSG or a fracture sim. Show the stump and spawn the cut piece as a rigid body so it drops with the ragdoll. Cosmetic only — hit resolution stays `combat/combat_rules.gd`. Depends on the ragdoll item for the falling piece.
14. **Horseback duel stage:** Both duelists fight mounted. Each horse’s speed varies randomly, so relative motion (and the shot window) changes from duel to duel. New arena/scenario; riding locomotion + aim-on-the-move on top of the existing standoff.
...
- **Practice hub:** Outdoor greybox range (`dev/build_practice_hub_blender.py` → `scenarios/practice_hub/`) with 12 breakable green bottles on rails at 6 / 11 / 16 m. Shoot them or grab and throw them; each respawns on its spot after 2 s. A HUD **Bottles N** counter tracks shatters, and a porch slot machine spins for flavor. No death, no persistence. **TUTORIAL / PRACTICE** on the main menu; flat menu sits on a random frozen arena, VR boots into the hub. Local only. (`done` in FEATURES)
