...
- Proximity voice chat in 1v1 multiplayer (LAN **and** Steam, Quest included): you hear the other player from their head in world space, so a standoff conversation is clear and someone who wanders off fades out. Talking is always on by default with a voice gate that keeps silence off the wire — switch on **Push To Talk** in Settings to only transmit while the bind is held (flat default **V**). **M** mutes your own mic, and a muted player wears a red **X** over their mouth so the other side can see it at a glance. Both binds are remappable in Settings → Controls; VR is always-on with the Settings toggle, because all six VR buttons are already combat binds.
...
- In-duel pause overlay (`ui/pause_menu.tscn`): Escape (flat) or VR menu button. Single-player pauses the game; multiplayer is overlay-only so the opponent is not frozen. Resume, Settings, Invite friends (Steam host), Restart Duel / Restart Gauntlet (full run; host-only in MP), Quit to main menu.
...
- A bottle in your hand on the practice range follows the camera smoothly, and a shot breaks it even while it is held. Horizon cacti on that lot use the same saguaro, barrel, and organ shapes as Canyon, and they sit in the sand instead of hovering above their shadows.
- Arm hits fling the revolver up out of the hand instead of snapping it to the hip. Catch it with either hand in VR, or look at it and press the draw button (right mouse) on the flat harness — in the air or on the ground. There is no redraw lock. AI duelists drop the gun the same way and pick it back up after a short beat. Debug panel **Regional Hits**: `gun_pain_toss_up`, `gun_pain_toss_spin`, `gun_flat_grab_range`, `gun_flat_grab_radius`. `arm_disarm_duration` is how long the AI waits before snatching the gun.
- Train Rooftop scenery scrolls faster (20 m/s, was 10). Horizon cliffs and buttes keep the same slower ratios.
- Canyon arena rebuilt from a Blender greybox (`assets/models/scenarios/canyon/canyon.glb`, headless `dev/build_canyon_blender.py`): winding cliff segments and rocks off the 22 m lane. The mesa is a narrow butte with a sheer drop; below it a 6 km desert with cacti, scrub, and distant mesas fades out in depth fog before the plain's edge. Canyon sun shadows fade out over a long distance so the cascade does not cut a line in front of those mesas. Dusk sky and sun stay; a light warm fog softens the far rock. Old CSG boxes removed. Detail meshes still planned.
- Saloon arena rebuilt from a Blender greybox (`assets/models/scenarios/saloon/saloon_interior.glb`, headless `dev/build_saloon_blender.py`): plank floor, west-wall bar and backbar, tables off the 10 m lane, swinging doors, and a north balcony under a 4.2 m ceiling. Old CSG boxes removed. Detail meshes still planned.
- Voice chat drops room tone instead of sending it: a cheap 180 Hz high-pass plus an RMS **Noise Gate** in Settings (default 0.08, same units as the mic-level bar — raise it until the bar sits still when you are quiet; 0 is off). Push-to-talk still ducks below the cutoff while the bind is held.
- Voice delay no longer piles up to several seconds. Capture keeps only the newest 40 ms after a hitch, playback is capped around 40–80 ms, and the mic stream rides its own net channel so a reliable shot cannot stall it.
...
- Meta Store / Quest APK is LAN-only multiplayer: Steam lobby buttons are hidden on Android, and `NetworkManager.steam_available()` stays false there. Steam lobbies remain a desktop / Steam SKU path.
